...
{/if*}
<p class="shopping_cart_contains">{l s='Your shopping cart contains:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></p>
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std" cellspacing="0" cellpadding="0">
		<thead>
			<tr>
				<th class="cart_product first_item"><div class="filler"></div><!--{l s='Product'}--></th>
				<th class="cart_description item"><div class="filler"></div><!--{l s='Description'}--></th>
				<!--<th class="cart_ref item">{l s='Ref.'}</th>-->
				<th class="cart_quantity item"><div class="filler"></div>{l s='Quantity'}</th>
				<th class="cart_unit item"><div class="filler"></div>{l s='Unit cost'}</th>
				<th class="cart_total item"><div class="filler"></div>{l s='Cost'}</th>
				<th class="cart_delete last_item"><div class="filler"></div></th>
			</tr>
		</thead>
		<tfoot>
		<!--{if $use_taxes}
		...
		{else}
			<tr class="cart_total_price">
				...
				<td colspan="2" class="price" id="total_product">{l s='TOTAL'} {displayPrice price=$total_products}</td>
			</tr>
		{/if}-->
			...
			<tr class="cart_total_voucher" {if $total_wrapping == 0}style="display: none;"{/if}>
                                <td><img src="/themes/prettypegs/img/prettypegs/prettypegs_gift_box.jpg" /></td>
				<td colspan="3">
                                    <p class="product_title">
                                        {if $use_taxes}
                                                {if $display_tax_label}{l s='Total gift-wrapping (tax incl.):'}{else}{l s='Total gift-wrapping:'}{/if}
                                        {else}
                                                {l s='Prettypegs gift box'}
                                        {/if}
                                    </p>
				</td>
			...
			<tr class="cart_total_price">
				...
				<td colspan="4" class="price" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</td>
			</tr>
			<tr class="cart_total_tax">
				<td colspan="3">{l s='Total tax:'}</td>
				<td colspan="4" class="price" id="total_tax">{displayPrice price=$total_tax}</td>
			</tr>
			{/if}
			<tr class="cart_shipping_price">
				<td colspan="1"><img src="{$carriers[0]['img']}" id="shipping_image" /></td>
				<td colspan="3" id="shipping_column">{l s='Your Pegs will be shipped by'} {$carriers[0]['name']}</td>
				<td colspan="1">
                                    {if $total_shipping_tax_exc <= 0}
					<span id="total_shipping" class="price">{l s='Free Shipping!'}</span>
                                    {else}
					<span id="total_shipping" class="price">{displayPrice price=$shippingCost}</span>
                                    {/if}
				</td>
				<td colspan="1">&nbsp;</td>
			</tr>
			<tr class="cart_total_price">
                                <td colspan="3">
                                        <p class="cart_navigation">
                                                {*{if !$opc}
                                                        <a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" class="exclusive standard-checkout next" title="{l s='Next'}">{l s='Next'} &raquo;</a>
                                                        {if Configuration::get('PS_ALLOW_MULTISHIPPING')}
                                                                <a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1" class="multishipping-button multishipping-checkout exclusive next" title="{l s='Next'}">{l s='Next'} &raquo;</a>
                                                        {/if}
                                                {/if}*}
									...
									<div class="customizationUploaded">
										<ul class="customizationUploaded">
											{foreach $custom_data as $picture}
												<li><img src="{$link->getImageLink($product.link_rewrite, $product.id_product, 'cart_product')}" alt="" class="customizationUploaded" /></li>
											{/foreach}
										</ul>
									</div>
		...
		{foreach $discounts as $discount}
			<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
				...
				<td class="price_discount_del">
					{if strlen($discount.code)}<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" title="{l s='Delete'}">{l s='X'}</a>{/if}
				</td>
			</tr>
		{/foreach}
		</tbody>
	{/if}
		...
		<p id="shopping_cart_footnote">{l s='Charged amount may differ slightly due to currency fluctuations.'}</p>
        <hr class="red_line" />
</div>
...
{if isset($empty)}
    <p class="cart_navigation">
	<a href="{$link->getModuleLink('klarnacheckout', 'checkout_klarna')}" class="exclusive standard-checkout next" title="{l s='Next'}">{l s='Next'} &raquo;</a>
                                            {*{if !$opc}
        ...
        {if $voucherAllowed}
                ...
                <form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                        <fieldset>
                                ...
                                <p class="voucher_note">{l s='If you have a voucher code please type it here.'}</p>
...
<!--<hr class="linethroughtop margin_bottom" />-->