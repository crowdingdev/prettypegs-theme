	...
	<div class="primary_block row" itemscope itemtype="http://schema.org/Product">
											...
											<div class="attribute_list">
												...
												<ul id="color_to_pick_list" class="clearfix">
													...
													<a href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" id="color_{$id_attribute|intval}" name="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}" style="background: {$colors.$id_attribute.value|escape:'html':'UTF-8'};" title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}">
														...
														<p>{$colors.$id_attribute.name|escape:'html':'UTF-8'}</p>
		...
		<div class="row">
			...
			<span style="">
				...
				<a href="{$link->getCategoryLink({$cat.id_category})}" title="{$cat.name}"> <h4> {$cat.name} </h4> </a>

				{foreach from=Product::getProducts($cookie->id_lang, 0,0, 'id_product', 'ASC', $cat.id_category) item=products}

				{foreach from=$products item=p}
	<div>
					{foreach from=Product::getProductName($p, null, null) item=name }
					{$name}
					{/foreach}
				</div>
				{/foreach}
